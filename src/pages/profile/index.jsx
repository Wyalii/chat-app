...
import { ADD_PROFILE_IMAGE_ROUTE, HOST, REMOVE_PROFILE_IMAGE_ROUTE, UPDATE_USER_PROFILE } from "@/utils/constants"
    ...
    const handleDeleteImage = async () =>{
        try {
            ...
            if(response.status===200){
                ...
                toast.success("image removed successffuly.")
                setImage(null)
            }
        } catch (error) {
            console.log({error})
             ...
             <div className="grid grid-cols-2">
                <div className="h-full w-32 md:w-48 md:h-48 relative flex items-center justify-center" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
                    <Avatar className = "h-32 w-32 md:w-48 md:h-48 rounded-full overflow-hidden">
                        {
                           image ? (<AvatarImage src = {image} alt = "Profile" className = "object-cover w-full h-full bg-black"></AvatarImage> ) :( 
                    ...
                    </Avatar>
                    {
                      ...
                      <input type = "file" ref={fileInputRef} className="hidden" onChange={handleImageChange} name="profile-image" accept="image/*"></input>  
                </div>
